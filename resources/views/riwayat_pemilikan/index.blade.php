        ...
        <tbody>
            @foreach ($riwayat_pemilikans as $riwayatPemilikan)
                <tr>
                    ...
                    <td>
                        <a class="btn btn-primary btn-sm" href="{{ route('riwayat_pemilikan.edit', $riwayatPemilikan->id) }}">Edit</a>
                        <form action="{{ route('riwayat_pemilikan.destroy', $riwayatPemilikan->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus riwayat pemilikan ini?')">Delete</button>
                        </form>
                    </td>
                </tr>
            @endforeach
